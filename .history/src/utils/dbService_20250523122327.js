...
export async function getSpendingByCategory(filters = {}, groupBy = 'main', userId) {
  ...
  if (groupBy === 'main') {
    ...
    transactions.forEach(t => {
      ...
      if (!spendingByMain[mainName]) {
        spendingByMain[mainName] = {
      ...
      spendingByMain[mainName].totalAmount += t.amount;
    });
    
    return Promise.resolve(Object.values(spendingByMain).sort((a, b) => b.totalAmount - a.totalAmount));
  } 
  else if (groupBy === 'sub') {
    // 중분류별 집계
    const spendingBySub = {};
    ...
    transactions.forEach(t => {
      ...
      if (!spendingBySub[subName]) {
        spendingBySub[subName] = {
      ...
      spendingBySub[subName].totalAmount += t.amount;
    });
    
    return Promise.resolve(Object.values(spendingBySub).sort((a, b) => b.totalAmount - a.totalAmount));
  } 
    ...
    transactions.forEach(t => {
      ...
      const detailName = detailCategory.name;
      
      if (!spendingByDetail[detailName]) {
        spendingByDetail[detailName] = {
      ...
      spendingByDetail[detailName].totalAmount += t.amount;
    });
    
    return Promise.resolve(Object.values(spendingByDetail).sort((a, b) => b.totalAmount - a.totalAmount));
  }
