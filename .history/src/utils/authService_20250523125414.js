...
const generateToken = (userData) => {
  ...
  const payload = {
    ...
    // 토큰 시크릿 키 생성 - 세션별 고유 키 생성
  let secretKey = localStorage.getItem('app_secret_key');
  if (!secretKey) {
    // 새로운 보안 키 생성
    const randomBytes = new Uint8Array(32);
    window.crypto.getRandomValues(randomBytes);
    secretKey = Array.from(randomBytes, b => b.toString(16).padStart(2, '0')).join('');
    localStorage.setItem('app_secret_key', secretKey);
  }
  // 추가적인 엔트로피를 위해 사용자 에이전트와 결합
  secretKey = secretKey + window.navigator.userAgent + window.screen.width + window.screen.height;
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (token) {
        ...
        // 토큰이 제공되지 않은 경우 로컬 스토리지에서 사용자 정보 제거
