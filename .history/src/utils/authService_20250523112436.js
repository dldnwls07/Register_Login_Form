 ...
 * 브라우저 환경과 Node.js 환경에서 모두 작동하도록 설계됨
 */

// 브라우저 환경인지 확인
const isBrowser = typeof window !== 'undefined';

// 브라우저 환경이면 Mock 서비스를 사용, 그렇지 않으면 실제 서비스 사용
import * as mockAuthService from './mockAuthService';

let executeQuery, bcrypt, jwt, JWT_SECRET, TOKEN_EXPIRY;

// 서버 환경에서만 Node.js 모듈 로드
if (!isBrowser) {
  // 실제 서버 환경에서만 필요한 모듈 임포트
  import('./dbService').then(module => { executeQuery = module.executeQuery });
  import('bcryptjs').then(module => { bcrypt = module.default });
  import('jsonwebtoken').then(module => { jwt = module.default });
  
  // JWT 시크릿 키 (실제 프로덕션에서는 환경 변수로 관리해야 함)
  JWT_SECRET = 'money-app-secret-key';
  TOKEN_EXPIRY = '24h';
}
