...
const hashPassword = (password) => {
  try {
    ...
    // 성능 개선을 위해 반복 횟수마다 해싱하지 않고, 청크로 나눠서 처리
    const chunkSize = 100; // 한 번에 100번씩 처리
    for (let chunk = 0; chunk < iterations; chunk += chunkSize) {
      ...
      // 성능을 위한 지연 방지 (브라우저 멈춤 현상 방지)
      if (chunk % 100 === 0 && chunk > 0) {
        // 작업 분할 시 콘솔 출력은 실제 환경에서는 제거
        if (process.env.NODE_ENV === 'development') {
          console.debug(`해싱 진행 중... ${chunk}/${iterations}`);
        }
      }
    }
