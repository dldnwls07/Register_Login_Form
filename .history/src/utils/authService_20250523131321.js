  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!usernameOrEmail || !password) {
        return reject(new AuthError('사용자 이름/이메일과 비밀번호를 모두 입력해주세요.', AUTH_ERROR_CODES.INVALID_CREDENTIALS));
      }
      
      // 계정 잠금 확인
      const lockStatus = checkLoginLock(usernameOrEmail);
      if (lockStatus.locked) {
        logSecurityEvent('login_attempt_while_locked', null, { identifier: usernameOrEmail });
        return reject(new AuthError(lockStatus.message, AUTH_ERROR_CODES.GENERAL_ERROR));
      }
      ...
      if (!user) {
        ...
        // 로그인 실패 추적
        const attemptStatus = trackLoginAttempt(usernameOrEmail, false);
        logSecurityEvent('login_failed', null, { reason: 'user_not_found', identifier: usernameOrEmail });
        
        return reject(new AuthError(attemptStatus.message, AUTH_ERROR_CODES.INVALID_CREDENTIALS));
      }
      ...
      if (!isPasswordValid) {
        ...
        // 로그인 실패 추적
        const attemptStatus = trackLoginAttempt(usernameOrEmail, false);
        logSecurityEvent('login_failed', user.id, { reason: 'invalid_password' });
