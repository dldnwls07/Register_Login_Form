...
export async function setFinancialGoal(goal, userId) {
  ...
  const result = await executeQuery(
    'INSERT INTO financial_goals (user_id, amount, start_date, end_date, is_achieved) VALUES (?, ?, ?, ?, ?)',
    [userId, amount, startDate, endDate, isAchieved || false]
  );
...
export async function getCurrentGoal(userId) {
  ...
  const goals = await executeQuery(
    'SELECT * FROM financial_goals WHERE user_id = ? AND ? BETWEEN start_date AND end_date ORDER BY created_at DESC LIMIT 1',
    [userId, today]
  );
