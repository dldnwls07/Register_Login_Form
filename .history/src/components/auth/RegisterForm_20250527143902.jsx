...
const RegisterForm = () => {
    ...
    const handleSendVerification = async (e) => {
        e.preventDefault();
        alert('[DEBUG] 인증코드 발송 버튼 클릭됨'); // 클릭 디버깅 확인용
        ...
        try {
            setSendingCode(true);
            console.log('[DEBUG] 인증코드 발송 요청 시작:', formData.email);
            
            // 직접 API 요청 - authService 대신 apiService 사용
            ...
            setErrors(prev => ({
                ...prev,
                email: error.response?.data?.message || '인증 코드 발송 중 오류가 발생했습니다'
            }));
        } finally {
    ...
    const handleVerifyCode = async (e) => {
        ...
        try {
            console.log('⭐️ 인증코드 확인 요청 시작:', {
                이메일: formData.email,
                코드: formData.verificationCode
            });
