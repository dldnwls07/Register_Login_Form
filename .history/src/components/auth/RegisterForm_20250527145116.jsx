...
const RegisterForm = () => {
    ...
    const handleSendVerification = async (e) => {
        ...
        try {
            setSendingCode(true);
            console.log('인증코드 발송 시작 - 직접 요청 - CORS 우회');
            
            // 문제 디버깅: 요청 URL, 메서드, 데이터 로깅
            const requestURL = `${api.defaults.baseURL}/auth/send-otp`;
            console.log(`요청 URL: ${requestURL}`);
            console.log(`요청 데이터:`, { email: formData.email });
            
            // 직접 XMLHttpRequest 사용 (크로스 브라우징 이슈 최소화)
            const xhr = new XMLHttpRequest();
            xhr.open('POST', requestURL, true);
            xhr.setRequestHeader('Content-Type', 'application/json');
            xhr.withCredentials = true; // CORS credentials 설정

            xhr.onload = function() {
                console.log('XHR 응답 상태:', xhr.status);
                console.log('XHR 응답 내용:', xhr.responseText);
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    let errorMsg = '인증 코드 발송 중 오류가 발생했습니다';
                    try {
            ...
            // 요청 전송
            xhr.send(JSON.stringify({ email: formData.email }));
            ...
            // 일정 시간 후에 sendingCode 상태 변경 (XHR은 비동기라서)
            setTimeout(() => {
