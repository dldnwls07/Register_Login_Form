  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (response && response.data && response.data.success) {
        // 성공 시 다음 단계로 진행하거나 사용자에게 알림
        setVerificationSuccess(true); // 인증 성공 상태 업데이트
        setSuccessMessage('이메일 인증에 성공했습니다. 다음 단계로 진행해주세요.');
        // 다음 단계로 자동 진행하거나, 사용자가 '다음' 버튼을 누르도록 유도할 수 있습니다.
        // 예: setActiveStep((prevActiveStep) => prevActiveStep + 1);
      } else {
        // 실패 메시지 처리 (API 응답에 따라)
        setErrorMessage(response.data.error || '인증 코드가 올바르지 않습니다.');
        setVerificationSuccess(false);
      }
    } catch (error) {
      console.error('Email verification error:', error.response ? error.response.data : error.message);
      setErrorMessage(error.response?.data?.error || '인증 코드 확인 중 오류가 발생했습니다.');
      setVerificationSuccess(false);
    }
    setIsLoading(false);
  };
