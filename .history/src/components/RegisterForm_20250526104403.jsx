...
const RegisterForm = ({ onRegistrationSuccess }) => {
    ...
    const checkDuplicateUsername = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/auth/check-username?username=${formData.username}`);
            if (!response.ok) {
    ...
    const checkDuplicateEmail = async () => {
        ...
        try {
            const response = await fetch(`/api/auth/check-email?email=${formData.email}`);
            const data = await response.json();
            if (!data.available) {
