...
const StepperRegisterForm = () => {
  ...
  const verifyEmail = async () => {
    try {
      // Correct the field name from 'code' to 'verificationCode'
      await apiService.post('/auth/verify-email', {
        email: formData.email,
        verificationCode: formData.verificationCode // Changed 'code' to 'verificationCode'
      });
      setErrors(prev => ({ ...prev, verificationCode: '' })); // Clear specific error on success
      return true;
    } catch (error) {
      setErrors(prev => ({
        ...prev,
        verificationCode: error.response?.data?.error || error.response?.data?.message || '인증 코드가 일치하지 않습니다.'
      }));
      return false;
    }
  };
  ...
  const handleVerifyCode = async () => {
    if (!formData.verificationCode) { // Use formData.verificationCode
      setErrorMessage('인증 코드를 입력해주세요.'); // Use setErrorMessage
      return;
    }
    ...
    try {
      const response = await apiService.post('/auth/verify-email', {
        email: formData.email,
        verificationCode: formData.verificationCode, // Use formData.verificationCode and correct field name
      });
      setSuccessMessage(response.data.message || '이메일 인증에 성공했습니다.'); // Use setSuccessMessage for the message
      setVerificationSuccess(true); // Set the boolean state to true
    } catch (error) {
      console.error('Verification error:', error.response ? error.response.data : error.message);
      setErrorMessage(
        error.response?.data?.error || error.response?.data?.message || '인증 코드 확인 중 오류가 발생했습니다.'
      );
      setVerificationSuccess(false); // Set boolean to false
    } finally {
      setIsLoading(false);
    }
  };
