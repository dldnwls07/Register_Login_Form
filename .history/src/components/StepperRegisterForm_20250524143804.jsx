...
const StepperRegisterForm = () => {
  ...
  const checkUsername = async () => {
    try {
      await apiService.post('/auth/check-username', { username: formData.username });
      return true;
    } catch (error) {
      setErrors(prev => ({
        ...prev,
        username: error.response?.data?.message || '사용자 이름 확인 중 오류가 발생했습니다.'
      }));
      return false;
    }
  };
    ...
    try {
      switch (step) {
        case 1:
          ...
          if (formData.username.length < 3) {
            setErrors({ username: '사용자 이름은 3자 이상이어야 합니다.' });
            return;
          }
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 2:
        return (
          <div className="form-group" role="group" aria-labelledby="email-label">
            ...
            />
            {errors.email && (
              <div id="email-error" className="error-text" role="alert"></div>
                {errors.email}
              </div>
            )}
          </div>
      ...
      case 3:
        return (
          <div className="form-group" role="group" aria-labelledby="verification-label"></div>
            ...
            <div className="verification-code-group">
              ...
              {countdown > 0 && (
                <div className="countdown" role="timer" aria-live="polite"></div>
