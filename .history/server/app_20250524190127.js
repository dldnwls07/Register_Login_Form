...
app.use((req, res, next) => {
  ...
  if (req.cookies && req.cookies['connect.sid']) {
    try {
      const rawSidCookie = req.cookies['connect.sid'];
      // 's:' 접두사와 '.' 이후의 서명을 제거하여 실제 세션 ID 추출 시도
      // URL 디코딩은 express-session이 내부적으로 처리하므로 여기서는 하지 않음
      // connect.sid 쿠키 값은 's:세션ID.서명' 형태임
      const sidParts = rawSidCookie.split('.');
      if (sidParts[0] && sidParts[0].startsWith('s:')) {
        const actualSid = sidParts[0].substring(2);
        console.log(`[PRE-SESSION] Client sent connect.sid, parsed actual session ID: ${actualSid}`);
      } else {
        console.log(`[PRE-SESSION] Client sent connect.sid, but format is unexpected: ${rawSidCookie}`);
      }
    } catch (e) {
      console.error('[PRE-SESSION] Error parsing connect.sid cookie:', e);
    }
  }
  next();
});
