...
const { Op } = require('sequelize'); // <--- Op 직접 임포트
...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({
    where: {
      [Op.or]: [ // <--- User.sequelize.Op 대신 Op 사용
        { email: email },
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({
    where: {
      ...
      resetPasswordExpire: { [Op.gt]: Date.now() } // <--- User.sequelize.Op 대신 Op 사용 (resetPassword 함수에서도 수정)
    }
  });
