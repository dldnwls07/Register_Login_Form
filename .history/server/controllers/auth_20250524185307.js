...
exports.sendVerificationEmail = asyncHandler(async (req, res, next) => {
  ...
  console.log('Verification code set in session:', req.session.verificationCode);
  console.log('Session ID after setting code:', req.sessionID);
  console.log('Full req.session after setting code:', req.session); // <--- 추가된 로그
  ...
  try {
    ...
    return; // 응답 후 함수 실행을 명확히 종료
  } catch (error) {
    console.error('이메일 발송 실패:', error);
    // next()가 중복 호출되지 않도록 헤더 전송 여부 확인 (선택적이지만 안전함)
    if (!res.headersSent) {
      return next(new ErrorResponse('이메일 발송에 실패했습니다. 나중에 다시 시도해주세요.', 500));
    } else {
      // 이미 응답이 보내진 경우, 추가 오류 로깅만 수행
...
exports.verifyEmail = asyncHandler(async (req, res, next) => {
  ...
  console.log('req.session.verificationCode (before check):', req.session.verificationCode); // <--- 추가된 로그

  // 세션에서 인증 코드 확인
  const verification = req.session.verificationCode;

  if (!verification || verification.email !== email || verification.code !== code) {
    // 실패 시에도 어떤 값으로 비교했는지 로그 추가
