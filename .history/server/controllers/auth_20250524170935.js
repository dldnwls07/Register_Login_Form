...
exports.register = asyncHandler(async (req, res, next) => {
  ...
  if (!passwordRegex.test(password)) {
    return next(new ErrorResponse('비밀번호는 최소 자 이상이며, 대문자, 숫자, 특수문자를 각각 하나 이상 포함해야 합니다.', 400));
  }
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  // 비밀번호 강도 검증 (register 함수와 동일한 로직)
  const passwordRegex = /^(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{10,}$/;
  if (!req.body.password || !passwordRegex.test(req.body.password)) {
    return next(new ErrorResponse('비밀번호는 최소 10자 이상이며, 대문자, 숫자, 특수문자를 각각 하나 이상 포함해야 합니다.', 400));
  }
