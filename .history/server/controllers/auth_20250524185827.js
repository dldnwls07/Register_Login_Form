...
exports.verifyEmail = asyncHandler(async (req, res, next) => {
  ...
  if (!verification || verification.email !== email || verification.code !== code) {
    console.error('Verification failed. Comparing:');
    console.error('Expected email:', email, ', Got:', verification ? verification.email : 'undefined');
    console.error('Expected code:', code, ', Got:', verification ? verification.code : 'undefined');
  ...
  // await new Promise((resolve, reject) => req.session.save(err => err ? reject(err) : resolve())); // 변경사항 저장

  console.log('Email verified successfully, req.session.verificationData:', req.session.verificationData);
  console.log('Full req.session after successful verification:', req.session);
