...
exports.verifyEmail = asyncHandler(async (req, res, next) => {
  ...
  console.log("req.session.verificationData (before check):", req.session.verificationData);
  console.log("Full req.session in verifyEmail:", req.session);

  const { email, verificationCode } = req.body; // 'code'에서 'verificationCode'로 변경
  const sessionVerificationData = req.session.verificationData;

  if (!sessionVerificationData) {
    return next(new ErrorResponse('세션에 인증 정보가 없습니다. 다시 시도해주세요.', 400));
  }

  if (
    sessionVerificationData.email === email &&
    sessionVerificationData.code === verificationCode && // 비교 시에도 verificationCode 사용
    sessionVerificationData.expires > Date.now()
  ) {
    req.session.verificationData.isVerified = true;
    console.log("Email verified successfully. Session data updated:", req.session.verificationData);
    res.status(200).json({ success: true, message: '이메일 인증 성공' });
  } else {
    console.log("Verification failed. Comparing:");
    console.log(`Expected email: ${sessionVerificationData.email} , Got: ${email}`);
    console.log(`Expected code: ${sessionVerificationData.code} , Got: ${verificationCode}`); // 비교 시에도 verificationCode 사용
    console.log(`Expiration check: ${sessionVerificationData.expires} > ${Date.now()} is ${sessionVerificationData.expires > Date.now()}`);
