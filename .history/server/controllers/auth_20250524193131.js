...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({
    where: {
      [Op.or]: [ // User.sequelize.Op 대신 직접 Op 사용
        { email: email },
...
// @desc    관리자 - 사용자 계정 잠금 해제
// @route   PUT /api/auth/admin/users/unlock/:id
// @access  Private/Admin
...
// @desc    테스트 - 세션 데이터 확인
// @route   GET /api/auth/test/session
// @access  Private
exports.testSession = asyncHandler(async (req, res, next) => {
...
// @desc    테스트 - 에러 던지기
// @route   GET /api/auth/test/error
// @access  Public
exports.throwError = asyncHandler(async (req, res, next) => {
  throw new Error('테스트용 에러입니다.');
});

// @desc    토큰 생성 및 응답 처리 (공통 함수)
// @route   N/A
// @access  N/A
const sendTokenResponse = (user, statusCode, res) => {
  ...
  // 쿠키 설정 (옵션: httpOnly, secure, sameSite 등)
  const options = {
    expires: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30일 후 만료
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production', // 프로덕션 환경에서는 secure 설정
    sameSite: 'Strict' // CSRF 보호를 위한 SameSite 정책
  };

  res.cookie('token', token, options);

  // 응답 데이터에서 비밀번호 제거 (보안상 이유)
