...
exports.register = asyncHandler(async (req, res, next) => {
    const { username, email, password, confirmPassword, isEmailVerified, emailVerificationToken } = req.body;

    // 기본 유효성 검사
    if (!username || !email || !password || !confirmPassword) {
        return next(new ErrorResponse('모든 필수 필드를 입력해주세요.', 400));
    }
    ...
    try {
        ...
        if (error.code === 11000) {
            ...
            return next(new ErrorResponse(`이미 사용 중인 ${field}입니다.`, 400));
        }
        next(error);
    }
});
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: '비밀번호 재설정',
      message: `비밀번호 재설정을 위해 다음 링크를 클릭해주세요: \n\n ${process.env.FRONTEND_URL}/reset-password/${resetToken}`
    });
