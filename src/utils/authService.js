...
const API_CALL_THROTTLE = 1000; // 1초
...
const canMakeApiCall = () => {
  const now = Date.now();
  if (now - lastApiCallTimestamp < API_CALL_THROTTLE) {
    return false;
  }
...
export const sendOtp = async (email) => {
    ...
    try {
        console.log('[DEBUG] sendOtp 호출 시작:', {
            email,
            apiUrl: api.defaults.baseURL
        });

        // 더 명확한 로깅을 위해 수정
        const fullUrl = `${api.defaults.baseURL}/auth/send-otp`;
        console.log('[DEBUG] 실제 요청 URL:', fullUrl);
        console.log('[DEBUG] 요청 데이터:', { email });

        const response = await api.post('/auth/send-otp', { email });
        console.log('[DEBUG] sendOtp 응답:', response.data);
        return response.data;
    } catch (error) {
        console.error('[DEBUG] sendOtp 에러:', {
        ...
        // 네트워크 에러인지 확인
        if (!error.response) {
            console.error('[DEBUG] 네트워크 오류 또는 서버 응답 없음');
        }
        throw error;
    }
};
...
export const verifyOtp = async (email, otp) => {
    ...
    try {
        const fullUrl = `${api.defaults.baseURL}/auth/verify-otp`;
        console.log('[DEBUG] verifyOtp API 요청:', {
            url: fullUrl,
            data: { email, otp }
        });

        const response = await api.post('/auth/verify-otp', { email, otp });
        console.log('[DEBUG] verifyOtp 응답:', response.data);
        return response.data;
    } catch (error) {
        console.error('[DEBUG] verifyOtp 에러:', {
            message: error.message,
            status: error.response?.status,
            statusText: error.response?.statusText,
            data: error.response?.data
