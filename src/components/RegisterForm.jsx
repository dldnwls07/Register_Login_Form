...
const RegisterForm = ({ onRegistrationSuccess }) => {
    ...
    const handleFinalSubmit = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                onRegistrationSuccess(); // 성공 시 콜백 함수 호출
            } else {
    ...
    const getPasswordStrength = (password) => {
        ...
        switch (score) {
            ...
            case 2:
                return { level: 'warning', text: '보통' };
            case 3:
            case 4:
                return { level: 'normal', text: '안전' };
            case 5:
                return { level: 'safe', text: '매우 안전' };
            default:
                return { level: '', text: '' };
        }
    };
                ...
                <div className={`form-step ${step === 4 ? 'active' : ''}`}>
                    <form onSubmit={handleFinalSubmit} className="password-form">
                        <div className="form-group">
                            ...
                            />
                            {formData.password && (
                                <div className={`password-strength strength-${getPasswordStrength(formData.password).level}`}>
